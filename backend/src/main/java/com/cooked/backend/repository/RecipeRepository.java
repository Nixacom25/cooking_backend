...
public interface RecipeRepository extends JpaRepository<Recipe, UUID> {
    ...
    @org.springframework.data.jpa.repository.Query("SELECT c.name, c.image, COUNT(r) FROM Recipe r JOIN r.category c WHERE r.origin = 'EXPLORE' AND c.type = 'CATEGORY' GROUP BY c.name, c.image ORDER BY COUNT(r) DESC")
    List<Object[]> findCategoriesWithCount();

    @org.springframework.data.jpa.repository.Query("SELECT c.name, c.image, COUNT(r) FROM Recipe r JOIN r.cuisine c WHERE r.origin = 'EXPLORE' AND c.type = 'CUISINE' GROUP BY c.name, c.image ORDER BY COUNT(r) DESC")
